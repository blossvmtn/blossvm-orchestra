...
| `riskTier` | `"R0"\|"R1"\|"R2"\|"R3"\|"R4"`, optional | new — field exists from P0 so P1's D8/D9 work has somewhere to land; omitted until P1 assigns capability risk tiers (CodeRabbit, PR #1 review, 2026-07-18: the table previously said "required" while the prose said "always unset" — the code (`TaskSpecSchema.riskTier` is `.optional()`) was always right, the table wasn't) |
| `createdAt` | ISO8601 | new |
...
2. Scaffold the four-package skeleton as a workspace: `packages/orchestra-core`, `packages/orchestra-daemon`, `packages/orchestra-cli`, `apps/orchestra-cockpit`. The existing Next.js app moves to `apps/orchestra-web-legacy` via `git mv` (a location change, not a rewrite — `npm run dev` inside that folder behaves identically, so D13 stays literally true).
   ...
   - `orchestra-daemon` — Bun entry point (`daemon.ts`). A long-lived **local HTTP server** on a fixed `127.0.0.1` port (e.g. `41417`, chosen to avoid common dev-server collisions). Every request requires a short-lived token (generated at daemon startup, written to `~/.orchestra/daemon.token`, read by both the cockpit and the CLI) in a header — the one piece of hardening a localhost port needs, since anything else running locally can otherwise reach it too. Responds to one `GET /ping` route. Spawns nothing yet.
   - `orchestra-cli` — scriptable entry point, separate from any GUI. A single `status` command that reads the token file and calls `GET /ping` with plain `fetch`. Works with or without Tauri running.
   - `orchestra-cockpit` — Tauri v2 + Vite + React scaffold. On launch, Tauri's Rust side spawns `bun run daemon.ts` as a **plain child process** (`std::process::Command`) — this is the one process D1 scopes Rust to supervise. This deliberately does **not** use Tauri's `externalBin`/sidecar-bundling feature, which expects a compiled, platform-suffixed binary — D3 defers compilation to P5, so that mechanism doesn't apply yet. Rust resolves the `bun` binary's absolute path explicitly (checked install locations, or a one-time settings prompt) rather than trusting inherited shell `PATH` — GUI-launched macOS apps get a minimal PATH that typically excludes a Homebrew install, and this is the single most likely P0 failure mode on JD's machine if left implicit. **The cockpit's UI calls the daemon directly with plain `fetch()`, exactly like the current app's tRPC calls, just pointed at `http://127.0.0.1:41417` instead — no relay through Rust for this traffic**, and no raw socket, since a webview cannot open one (a webview is a browser context — it only gets the connection types a website gets). This corrects the Phase 0 spec's original Unix-domain-socket choice, which the webview leg specifically could never have used; confirmed against Tauri's own documented sidecar examples, where localhost HTTP is the standard pattern for exactly this shape.
...
6. Write **two** end-to-end verifications, not one:
   ...
   - **The IPC path**: the same fixture `WorkIntent` is submitted from `orchestra-cockpit`'s UI, over a real authenticated `fetch()` call to `orchestra-daemon`'s local HTTP server, and the resulting `Receipt` is read back the same way. This is the actual architecture bet Phase 0 exists to de-risk (a real schema surviving the real Tauri↔Bun boundary) — the contract path alone never touches it.
...
- [ ] A fixture `WorkIntent`, submitted from the real Tauri cockpit UI over a real authenticated `fetch()` call to the daemon's local HTTP server, reaches a stored `Receipt` — zero model calls, zero real git operations, zero real agent CLI involvement, but a genuine IPC round-trip, not a simulated one.
...
**2026-07-18 — IPC transport corrected before P1 (JD-requested review of the F3 resolution; corrected from a mistaken 2026-07-19 dateline, CodeRabbit PR #1 review — `git log` confirms commit `b737c4a` landed 2026-07-18):** the Unix-domain-socket answer to F3 had its own hole — a webview cannot open a raw socket, only a native process can, so "the cockpit talks to the daemon directly over the socket" was never actually buildable as written. Replaced with a token-authenticated local HTTP server (`daemon` listens on `127.0.0.1`, cockpit and CLI both use plain `fetch()`), confirmed against Tauri's own documented sidecar patterns as the standard shape for this exact problem. Rust's role is unchanged — spawn and supervise the one daemon process; it is no longer in the data path for the cockpit's calls to the daemon at all.
