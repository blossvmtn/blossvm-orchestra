...
**2026-07-19 — IPC transport corrected before P1 (JD-requested review of the F3 resolution):** the Unix-domain-socket answer to F3 had its own hole — a webview cannot open a raw socket, only a native process can, so "the cockpit talks to the daemon directly over the socket" was never actually buildable as written. Replaced with a token-authenticated local HTTP server (`daemon` listens on `127.0.0.1`, cockpit and CLI both use plain `fetch()`), confirmed against Tauri's own documented sidecar patterns as the standard shape for this exact problem. Rust's role is unchanged — spawn and supervise the one daemon process; it is no longer in the data path for the cockpit's calls to the daemon at all.
