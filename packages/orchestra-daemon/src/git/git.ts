...
/**
 * Ported near-verbatim from apps/orchestra-web-legacy/src/server/orchestra/git.ts
 * (Phase 1 spec, D23) — a pure execFile wrapper with zero coupling to the
 * legacy app's JSON-file state model, so it drops into this package
 * unchanged. Run git with argv only — never shell-interpolate paths.
...
export async function resolveDefaultBaseBranch(repoRoot: string): Promise<string> {
  ...
  try {
    const ref = await gitStdout(repoRoot, ["symbolic-ref", "refs/remotes/origin/HEAD"]);
  ...
  for (const candidate of ["main", "master"]) {
    try {
      await gitStdout(repoRoot, ["rev-parse", "--verify", `refs/heads/${candidate}`]);
      return candidate;
    } catch {
      // try next
    }
